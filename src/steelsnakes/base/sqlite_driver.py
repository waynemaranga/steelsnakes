...
def build_regional_sqlite_db(db_path: pathlib.Path, source_dir: pathlib.Path) -> pathlib.Path:
    """
    Build a single SQLite database containing one table per UK JSON file.
    Pass in the path to the database file and the path to the source directory. 
...
if __name__ == "__main__":
    SOURCE_DIR = pathlib.Path(__file__).parent.parent.absolute() / "UK" / "data"
    DB_DIR = SOURCE_DIR.parent / "UK_sections.sqlite3"
    print(SOURCE_DIR.is_dir()) # .is_dir() implies .exists()

    build_regional_sqlite_db(db_path=DB_DIR, source_dir=SOURCE_DIR)
    
    # built = build_regional_sqlite_db()
