...
def W(designation: str) -> WideFlangeBeam:
    ...
    return cast(WideFlangeBeam, get_US_Metric_factory().create_section(designation.upper().strip(), SectionType.W))

def S(designation: str) -> StandardBeam:
    """Note: case insensitive - accepts both "x" and "X" separators"""
    return cast(StandardBeam, get_US_Metric_factory().create_section(designation.upper().strip(), SectionType.S))

def M(designation: str) -> MiscellaneousBeam:
    """Note: case insensitive - accepts both "x" and "X" separators"""
    return cast(MiscellaneousBeam, get_US_Metric_factory().create_section(designation.upper().strip(), SectionType.M))

if __name__ == "__main__":
    print(W("W690X500").get_properties())
    print(S("S610X134").get_properties())
    print(M("M318X18.5").get_properties())