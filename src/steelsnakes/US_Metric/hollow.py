...
def HSS_RCT(designation: str) -> RectangularHSS:
    ...
    return cast(RectangularHSS, get_US_Metric_factory().create_section(designation.upper().strip(), SectionType.HSS_RCT))

def HSS_SQR(designation: str) -> SquareHSS:
    """Note: Case insensitive - accepts both "x" and "X" separators"""
    return cast(SquareHSS, get_US_Metric_factory().create_section(designation.upper().strip(), SectionType.HSS_SQR))

def HSS_RND(designation: str) -> RoundHSS:
    """Note: Case insensitive - accepts both "x" and "X" separators"""
    return cast(RoundHSS, get_US_Metric_factory().create_section(designation.upper().strip(), SectionType.HSS_RND))

if __name__ == "__main__":
    print(HSS_RCT("HSS304.8X101.6X6.4").get_properties())
    print(HSS_SQR("HSS101.6X101.6X4.8").get_properties())
    print(HSS_RND("HSS101.6X5.6").get_properties())