...
class USMetricSectionDatabase(SectionDatabase):
    ...
    def _resolve_data_directory(self, data_directory: Optional[Path]) -> Path:
        """Resolve the US(Metric) data directory path."""
        if data_directory is not None:
            return data_directory
            
        # Auto-discovery for US(Metric) sections
        current_file: Path = Path(__file__).resolve()
        possible_paths: list[Path] = [          
            Path.cwd() / "src/steelsnakes/US_Metric/data/", # from project root
            current_file.parent / "data/", # from package installation
            current_file.parent.parent.parent / "data/US_Metric/", # from development environment
            current_file.parent.parent.parent / "src/steelsnakes/US_Metric/data/", # from source directory
            current_file.parent.parent.parent.parent / "data/US_Metric/" # from parent directory
        ]
    ...
    def get_supported_types(self) -> list[SectionType]:
        """Return all US-Metric-supported section types."""
        return [
            # Beams
    ...
    def _fuzzy_find_section(self, designation: str) -> Optional[tuple[SectionType, dict[str, Any]]]:
        """
        US(Metric)-specific fuzzy section finding with case-insensitive matching.
        ...
        for section_type in self.get_supported_types():
            ...
            # TODO: handle SLBB & LLBB cases i.e exclude one from the other's fuzzy search

        return None

# Global instance for convenience
_global_US_Metric_database: Optional[USMetricSectionDatabase] = None


def get_US_Metric_database(data_directory: Optional[Path] = None, use_sqlite: bool = False) -> USMetricSectionDatabase:
    """Get or create global US(Metric) database instance. Uses JSON by default."""
    global _global_US_Metric_database
    if _global_US_Metric_database is None or data_directory is not None:
        _global_US_Metric_database = USMetricSectionDatabase(data_directory, use_sqlite=use_sqlite)
    return _global_US_Metric_database

if __name__ == "__main__":
    db: USMetricSectionDatabase = get_US_Metric_database()
