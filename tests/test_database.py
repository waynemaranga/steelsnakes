...
# from steelsnakes.base.sqlite3db import build_regional_sqlite_db, SQLiteJSONInterface
...
#     def __init__(self, data_directory: Optional[Path] = None, use_sqlite: bool = False, region: str = "MOCK"):
#         # Set up mock data directory before calling super().__init__
#         self._mock_data_directory = data_directory or Path(__file__).parent / "mock_data"
#         super().__init__(data_directory=data_directory, region=region, use_sqlite=use_sqlite)
    
#     def _resolve_data_directory(self, data_directory: Optional[Path], region: str) -> Path:
#         """Override to use mock data directory."""
...
#     # Mock SQLite methods for testing
#     def _get_sqlite_db_path(self) -> Path:
#         """Mock SQLite database path."""
#         if not hasattr(self, '_sqlite_db_path') or self._sqlite_db_path is None:
#             self._sqlite_db_path = self._mock_data_directory / "test.sqlite3"
